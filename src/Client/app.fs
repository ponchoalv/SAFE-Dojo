...
type ResultReportPromise =
    { LocationProm : Import.JS.Promise<LocationResponse>
      CrimesProm : Import.JS.Promise<CrimeResponse array>
      WeatherProm : Import.JS.Promise<WeatherResponse> }
...
module RequestHelper =
    [<PassGenerics>]
    let postRequestAs<'T,'B> uri (requestData:'T) options =
        Fetch.postRecord uri requestData options
        |> Promise.bind (fun fetched -> fetched.text())
        |> Promise.map ofJson<'B>

let getResponse postcode =
    promise { 
        let! location = postRequestAs<PostcodeRequest, LocationResponse> "api/distance" { Postcode = postcode } []
        
        let crimes = postRequestAs<Location, CrimeResponse array> "api/crime" location.Location []
        
        let weather = postRequestAs<Location, WeatherResponse> "api/weather" location.Location []
        
        (* it's better for async to call let and then let!, this way all post request 
        will be sent at the same time, and then, we block for the response.*)
        let! weather = weather
        let! crimes = crimes
...
let update msg model =
    ...
    | _, GotReport response -> 
        { model with ValidationError = None
                     Report = Some response
                     ServerState = Idle }, Cmd.none
    | _, PostcodeChanged p -> 
        let p = p.ToUpper()
        { model with Postcode = p
...
module ViewParts =
    let basicTile title options content =
        Tile.tile options 
            [ Notification.notification [ Notification.Props [ Style [ Height "100%"
                                                                       Width "100%" ] ] ] 
    ...
    let crimeTile crimes =
        ...
        basicTile "Crime" [] [ barChart [ Chart.Data cleanData
                                          Chart.Width 600.
                                          Chart.Height 500.
                                          Chart.Layout Vertical ] [ xaxis [ Cartesian.Type "number" ] []
    ...
    let getBingMapUrl latLong =
        sprintf "https://www.bing.com/maps/embed?h=400&w=800&cp=%f~%f&lvl=11&typ=s&FORM=MBEDV8" latLong.Latitude 
            latLong.Longitude
    
    let bingMapTile (latLong : LatLong) =
        let bingUrl = getBingMapUrl latLong
        basicTile "Map" [ Tile.Size Tile.Is12 ] [ iframe [ Style [ Height 410
                                                                   Width 810 ]
                        ...
                        [ div [] 
                              [ Level.heading [] 
                                    [ Image.image [ Image.Is128x128 ] 
                                          [ img 
                                                [ Src
                                                      (sprintf "https://www.metaweather.com/static/img/weather/%s.svg" 
    ...
    let locationTile model =
        childTile "Location" 
            [ div [] [ Heading.h3 [] [ str model.Location.Location.Town ]
...
let view model dispatch =
    div [] 
        [ Navbar.navbar [ Navbar.Color IsPrimary ] [ Navbar.Item.div [] [ Heading.h1 [] [ str "Location Review!" ] ] ]
          ...
          Container.container [] 
              [ yield Field.div [] 
                          [ Label.label [] [ str "Postcode" ]
                            ...
                            Control.div [ Control.HasIconLeft; Control.HasIconRight ] 
                                [ Input.text [ Input.Placeholder "Ex: EC2A 4NE"
                                               ...
                                               Input.Color(if model.ValidationError.IsSome then Color.IsDanger
                                                           else Color.IsSuccess)
                                               Input.Props [ OnChange
                                                                 (fun ev -> dispatch (PostcodeChanged !!ev.target?value))
                                                             onKeyDown KeyCode.enter (fun _ -> dispatch GetReport) ] ]
                                  Icon.faIcon [ Icon.Size IsSmall
                                                Icon.IsLeft ] [ Fa.icon Fa.I.Building ]
                                  (match model with
                                   | { ValidationError = Some _ } -> 
                                       Icon.faIcon [ Icon.Size IsSmall
                                                     Icon.IsRight ] [ Fa.icon Fa.I.Exclamation ]
                                   | { ValidationError = None } -> 
                                       Icon.faIcon [ Icon.Size IsSmall
                                                     Icon.IsRight ] [ Fa.icon Fa.I.Check ]) ]
                            
                            Help.help [ Help.Color(if model.ValidationError.IsNone then IsSuccess
                                                   else IsDanger) ] 
                ...
                | { Report = Some model } -> 
                    yield Tile.ancestor [] 
                              [ Tile.parent [ Tile.Size Tile.Is12 ] [ bingMapTile model.Location.Location.LatLong ] ]
                    yield Tile.ancestor [] [ Tile.parent [ Tile.IsVertical
                                                           Tile.Size Tile.Is4 ] [ locationTile model
                               ...
                               from the model, and include it here as part of the list *)
                                             ...
                                             Tile.parent [ Tile.Size Tile.Is8 ] [ crimeTile model.Crimes ] ] ]
          br []
          
          Footer.footer [] 
              [ Content.content [ Content.CustomClass Bulma.Properties.Alignment.HasTextCentered ] [ safeComponents ] ] ]
